...
##def get_hotel_page(hotel_name, input_url, city, state, datadir):
##    """ Returns the URL of the list of the hotels in a city. Corresponds to
##    STEP 1 & 2 of the slides.
##
##    Parameters
...
##    hotel_name = "/Hotel_Review-g60745-d94330-Reviews-" + hotelname+ "-Boston_Massachusetts.html"
##    soup = BeautifulSoup(html)
##     #div  = soup.find("div",{"class": "listing_title"})       
##    hotel_url = soup.find('a', href=hotelname)
##    return hotel_url['href']

##def scrape_hotel_info(html):
##    """Parses the website with the hotel list and prints the hotel name, the
##    number of stars and the number of reviews it has. If there is a next page
##    in the hotel list, it returns a list to that page. Otherwise, it exits the
##    script. Corresponds to STEP 4 of the slides.
...
##    soup = BeautifulSoup(html)
##    # Extract hotel name, star rating and number of reviews
##    hotel_boxes = soup.findAll('div', {'class' :'heading_name_wrapper'})
##    if not hotel_boxes:
##        log.info("#################################### Option 2 ######################################")
##        hotel_boxes = soup.findAll('div', {'class' :'listing_info jfy'})
##    if not hotel_boxes:
...
##        hotel_url = base_url + hotel_box.find("a",{"target" :"blank})_.get('href')
##        get_hotel_page(hotel_url)                                           
##                                                   
##                                                   
## col20f2 composite  ,trip_type, wrap subrating  
##        traveler_rating = hotel_box.find("img", {"class" : "col20f2 composite"})
####        if stars:
####            log.info("traveler_rating: %s" % stars['alt'].split()[0])
##
##        num_reviews = hotel_box.find("span", {'class': "trip_type"}).findAll(text=True)
####        if num_reviews:
####            log.info("Number of reviews: %s " % [x for x in num_reviews if "review" in x][0].strip())
